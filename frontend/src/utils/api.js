...
export const apiRequest = async (endpoint, options = {}) => {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Server returned HTML instead of JSON. Check VITE_API_URL setting. Response: ${text.substring(0, 100)}`);
    }
...
export const queueAPI = {
  ...
  join: (driverId, vehicleId) => apiRequest('/queue/join', {
    method: 'POST',
    body: JSON.stringify({ driver_id: driverId, vehicle_id: vehicleId || undefined })
  }),
  dispatch: (id, opts = {}) => apiRequest(`/queue/dispatch/${id}`, {
    method: 'POST',
    body: JSON.stringify({
  ...
  registerTrip: (driverId, vehicleId) => apiRequest('/queue/register-trip', {
    method: 'POST',
    body: JSON.stringify({ driver_id: driverId, vehicle_id: vehicleId || undefined })
  }),
  getDrivers: () => apiRequest('/users/drivers/list'),
  getTraffic: (origin, destination) =>
    apiRequest(`/queue/traffic?origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(destination)}`)
};
...
export const fareAPI = {
  ...
  getRoute: (origin, destination, passengers = 1) =>
    apiRequest(`/fares/route?origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(destination)}&passengers=${passengers}`)
};
